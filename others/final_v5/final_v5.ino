...
#define ZHONGZHI -2.5 //小车的机械中值 
#define DIFFERENCE 5
#define PIN 14  // PIN为不存在的管脚，防止冲突
// 因为还没测试，不知道可不可以共用4号管脚
#define MAX_LED 7
#define ADD true
#define SUB false
...
float Balance_Kp = 10, Balance_Kd = 0.3, Velocity_Kp = 2, Velocity_Ki = 0.01;
float turn_kp = 2, turn_kd = 0.001;  // 转向环kp和kd值
//  b_kp=15,  b_kd=0.5
//  v_kp = 2, v_ki = 0.01

MPU6050 Mpu6050;  // 实例化MPU6050
...
void setup() {
  ...
  pinMode(ENCODER_L, INPUT);      // 3
  pinMode(DIRECTION_L, INPUT);      // 8

  pinMode(PWMB, OUTPUT);    // 电机PWMB
  pinMode(BIN1, OUTPUT);    // BIN1，控制电机2的方向，10为正转，01为反转
  pinMode(BIN2, OUTPUT);    // BIN2
  pinMode(ENCODER_R, INPUT);      // 4
  pinMode(DIRECTION_R, INPUT);      // 5
  ...
  Mpu6050.setXGyroOffset(81);  // 81
  Mpu6050.setYGyroOffset(-413);   // -413
  Mpu6050.setZGyroOffset(36);  // 36
...
{
  int Amplitude = 150;  //===PWM满幅是255 限制在150
...
void control() {
  ...
  Motor1 = Balance_Pwm - Velocity_Pwm - Turn_Pwm;  // + turn_pwm
  Motor2 = Balance_Pwm - Velocity_Pwm + Turn_Pwm;  // - turn_pwm



  Xianfu_Pwm();  // 限幅

  Set_Pwm(Motor1, Motor2);  // pwm赋值
